...
type Props = {
    ...
    onView?: (event: PortalEventCard) => void;
};

const EventListItem = ({ event, onView }: Props) => {
    return (
        <Card className={"w-full flex flex-row gap-0 p-4 justify-between items-center shadow-xs"}>
            <div className={"flex items-start lg:items-start gap-2"}>
                ...
                <div>
                    ...
                    <p className={"text-xs text-muted-foreground mb-1"}>{event.start} | {event.venue}</p>
                    {event.guest && (
            ...
            <div className={"flex items-center gap-2"}>
                <Badge>{event.free ? "Free" : `TZS ${event.price.toLocaleString()}`}</Badge>
